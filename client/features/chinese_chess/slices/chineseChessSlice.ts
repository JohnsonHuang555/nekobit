...
const findChessByLocation = (chesses: ChineseChess[], x: number, y:number): ChineseChess => {
  return chesses.find((chess: ChineseChess) => (
    chess.locationX === x && chess.locationY === y
  )) as ChineseChess;
}

const checkMove = (chesses: ChineseChess[], selectedChess: ChineseChess, targetX: number, targetY: number): boolean => {
  let canMove = false;
    switch (selectedChess.name) {
        ...
        if (selectedChess.locationX === targetX) {
          for (let i = 0; i < Math.abs(selectedChess.locationY - targetY) - 1; i++) {
            ...
            if (targetY > selectedChess.locationY) {
              hasChessY = findChessByLocation(chesses, targetX, selectedChess.locationY + i + 1);
            } else {
          ...
          for (let i = 0; i < Math.abs(selectedChess.locationX - targetX) - 1; i++) {
            ...
            if (targetX > selectedChess.locationX) {
              hasChessX = findChessByLocation(chesses, selectedChess.locationX + i + 1, targetY);
            } else {
      ...
      case ChessName.HorsesRed: {
        ...
        ['xAdd', 'xMinus', 'yAdd', 'yMinus'].forEach(item => {
          switch (item) {
            case 'xAdd':
              // 拐馬腳
              const xAddObstacle = findChessByLocation(chesses, selectedChess.locationX + 1, selectedChess.locationY);
              ...
              break;
            case 'xMinus':
              const xMinusObstacle = findChessByLocation(chesses, selectedChess.locationX - 1, selectedChess.locationY);
              ...
              break;
            case 'yAdd':
              const yAddObstacle = findChessByLocation(chesses, selectedChess.locationX, selectedChess.locationY + 1);
              ...
              break;
            case 'yMinus':
              const yMinusObstacle = findChessByLocation(chesses, selectedChess.locationX, selectedChess.locationY - 1);
      ...
      case ChessName.MinisterRed: {
        ...
        ['left', 'right'].forEach(item => {
          switch (item) {
            case 'left':
              const topLeftObstacle = findChessByLocation(chesses, selectedChess.locationX - 1, selectedChess.locationY - 1);
              const bottomLeftObstacle = findChessByLocation(chesses, selectedChess.locationX - 1, selectedChess.locationY + 1);
              ...
              break;
            case 'right':
              const topRightObstacle = findChessByLocation(chesses, selectedChess.locationX + 1, selectedChess.locationY - 1);
              const bottomRightObstacle = findChessByLocation(chesses, selectedChess.locationX + 1, selectedChess.locationY + 1);
