...
import {
  ...
  selectCheckMate,
  selectGameData,
} from "../selectors/chineseChessSelector";
...
const ChineseChessContainer = () => {
  ...
  // const checkMate = useSelector(selectCheckMate);
  ...
  // const playersId = selectedRoom.playerList.map((p: any) => {
  //   return p.id;
  // });
  ...
  //     (c: any) => c.alive && c.side === ChessSide.Black
  //   );
  //   const redChesses = chineseChess.filter(
  //     (c: any) => c.alive && c.side === ChessSide.Red
  //   );
  //   if (
  ...
  return (
    <>
      ...
      >
        ...
        />
        <Button title="離開" color="grey-4" onClick={() => onLeave()} />
      </Modal>
      <Modal
      ...
      <div className={styles.chineseChess}>
        ...
        <div className={styles.footer}>
          {isGameOver && !showGameOverModal ? (
            <Button title="結束" color="brown" onClick={() => onLeave()} />
          ) : (
            <Button
              title="投降"
              color="brown"
